...
for i in tqdm(range(num_epochs)):
    ...
    for moon in tqdm(moons):
        # pdb.set_trace()
        X_moon = torch.from_numpy(X[X.date == moon].drop(columns=["date"]).values).to(
            torch.float32
        )
        y_moon = torch.from_numpy(Y[X.date == moon].values.reshape(-1, 1)).to(
            torch.float32
        )

        b_moon = torch.from_numpy(B[B.date == moon].drop(columns=["date"]).values).to(
            torch.float32
        )
        # import pdb
        # pdb.set_trace()
...
print(
    loss_mse(
        pred_train, torch.from_numpy(Y.values[:, -1].reshape(-1, 1)).to(torch.float32)
    ).item()
)
