...
class TimeSeriesSplitGroups(_BaseKFold):
    ...
    def split(self, X, y=None, groups=None):
        ...
        indices = np.arange(
            n_samples
        )  # array([    0,     1,     2, ..., 50027, 50028, 50029])
        test_size = n_groups // n_folds  # floor division of index --> 50/6 ~ 8
        test_starts = range(
            test_size + n_groups % n_folds, n_groups, test_size  # range(10, 50, 8)
        )
        test_starts_list = list(test_starts)[::-1]  # [42, 34, 26, 18, 10]
        for test_start in test_starts_list:  # [42, 34, 26, 18, 10] # 5 splits -->
            yield (
